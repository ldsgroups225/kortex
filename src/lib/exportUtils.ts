...
// Export functions
export function exportNotesAsMarkdown(notes: any[]): string {
...
export function exportNotesAsJson(notes: any[]): string {
  const exportData = {
    notes,
...
export function exportSnippetsAsJson(snippets: any[]): string {
  const exportData = {
    snippets,
    exportDate: new Date().toISOString(),
    version: '1.0',
...
export function exportSnippetsAsCodeBlocks(snippets: any[]): string {
...
export function exportTodosAsJson(todos: any[]): string {
  const exportData = {
    todos,
    exportDate: new Date().toISOString(),
    version: '1.0',
...
export async function exportAllAsZip(data: ExportData): Promise<Blob> {
  const zip = new JSZip()
...
// Import functions
export function validateImportData(data: any): ImportData | null {
  try {
