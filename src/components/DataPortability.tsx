...
export function DataPortability() {
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      console.log('Importing data:', importPreview);
      alert('Import functionality will be implemented with Convex mutations');
    } catch (error) {
      console.error('Import failed:', error);
      alert('Import failed');
    } finally {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="p-6">
          ...
          <div className="space-y-4">
            ...
            <div>
              <h3 className="text-md font-medium text-gray-900 dark:text-white mb-2">
                {t('snippets.title')} ({snippets.length})
              </h3>
              <div className="flex gap-2">
                <button
            ...
            <div>
              <h3 className="text-md font-medium text-gray-900 dark:text-white mb-2">
                {t('todos.title')} ({todos ? todos.todo.length + todos.inProgress.length + todos.done.length : 0})
              </h3>
              <div className="flex gap-2">
                <button
            ...
            <div className="p-6 space-y-4 max-h-96 overflow-y-auto">
              ...
              {importPreview.notes && importPreview.notes.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span>{importPreview.notes.length} notes</span>
                </div>
              )}
              ...
              {importPreview.snippets && importPreview.snippets.length > 0 && (
                <div className="flex items-center gap-2">
                  <CheckIcon className="h-4 w-4 text-green-500" />
                  <span>{importPreview.snippets.length} snippets</span>
                </div>
              )}
              ...
              {importPreview.todos && importPreview.todos.length > 0 && (
                <div className="flex items-center gap-2">
                  <CheckIcon className="h-4 w-4 text-green-500" />
                  <span>{importPreview.todos.length} todos</span>
                </div>
              )}

              {importPreview.exportDate && (
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  Originally exported: {new Date(importPreview.exportDate).toLocaleString()}
                </div>
              )}
            </div>
